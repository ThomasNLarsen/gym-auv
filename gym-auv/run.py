...
from stable_baselines.ddpg import AdaptiveParamNoiseSpec, NormalActionNoise, LnMlpPolicy, OrnsteinUhlenbeckActionNoise
...
def main(args):
    ...
    if args.mode == 'play':
            ...
            if (model == PPO2):
                if args.recurrent:
                    ...
                    hyperparams = {
                        ...
                        'lam': 0.98,           # Default 0.95
                        'gamma': 0.999,        # Default 0.99
                        'noptepochs': 4,       # Default 4
        ...
        def callback(_locals, _globals):
            ...
            elif model == A2C or model == ACER or model == ACKTR:  # or model == TRPO:
                save_criteria = n_updates % 100 == 0
                recording_criteria = n_updates % 1000 == 0
                report_criteria = True
                if save_criteria:
                    _self.save(agent_filepath)
            elif model == DDPG or model == TD3 or model == SAC or model == TRPO:
        ...
        set_global_seeds(0)  # Thomas 04.08.21 --- Ensuring deterministic order of environments for fair intercomparison and plots.
